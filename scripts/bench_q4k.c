/* bench_q4k.c — micro-benchmark Q4_K matmul kernel
 * Build: gcc -O3 -mavx2 -mfma -mf16c -I. -o bench_q4k scripts/bench_q4k.c -lm
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
/* Simple Q4_K quantization (matches Python converter, INTERLEAVED packing) */
...
int main() {
    ...
    for (int t = 0; t < n_tests; t++) {
        ...
        for (int j = 0; j < out_dim; j++)
            quantize_q4_k_row(w + j*in_dim, q4k + (size_t)j*row_stride, in_dim);
        ...
        /* Memory read: out_dim rows × row_stride bytes (weights) + in_dim*4 (x) */
        double mem_read = (double)out_dim * row_stride + in_dim * 4;
        /* FLOPS: 2 * out_dim * in_dim (multiply-add) */
        double flops = 2.0 * out_dim * in_dim;
        printf("%-22s  %6.2f ms  %5.1f GB/s  %5.1f GFLOPS\n",
               tests[t].name, dt*1000, mem_read/dt/1e9, flops/dt/1e9);
