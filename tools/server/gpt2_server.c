 ...
 * Macro stubs only active when AVX2 is not available. */
  ...
  #define _mm256_setzero_si256() (__stub_zero256i)
  ...
  #define _mm256_extracti128_si256(v, i) (__stub_zero128i)
  #define _mm256_castps256_ps128(v) (__stub_zero128)
  #define _mm256_extractf128_ps(v, i) (__stub_zero128)
  #define _mm256_hadd_ps(a, b) (__stub_zero256)
  #define _mm256_inserti128_si256(v, x, i) (__stub_zero256i)
  #define _mm256_castsi128_si256(v) (__stub_zero256i)
  #define _mm256_cvtepi8_epi32(v) (__stub_zero256i)
  #define _mm256_cvtepi32_ps(v) (__stub_zero256)
  #define _mm_add_ps(a, b) (__stub_zero128)
  #define _mm_hadd_ps(a, b) (__stub_zero128)
  #define _mm_cvtss_f32(v) 0.0f
  #define _mm_load_si128(p) (__stub_zero128i)
  #define _mm_loadu_si128(p) (__stub_zero128i)
  #define _mm_set1_epi8(x) (__stub_zero128i)
  #define _mm_and_si128(a, b) (__stub_zero128i)
  #define _mm_srli_epi16(v, n) (__stub_zero128i)
  #define _mm_sub_epi8(a, b) (__stub_zero128i)
  #define _mm_unpacklo_epi8(a, b) (__stub_zero128i)
  #define _mm_unpackhi_epi8(a, b) (__stub_zero128i)
  #define _mm_add_epi32(a, b) (__stub_zero128i)
  #define _mm_hadd_epi32(a, b) (__stub_zero128i)
...
static void compute_q4_w_sums(const uint8_t *q4_T, int32_t *w_sums, int in_dim, int out_dim) {
    int half = in_dim / 2;
    for (int j = 0; j < out_dim; j++) {
        int32_t s = 0;
        const uint8_t *w = q4_T + (size_t)j * half;
        for (int i = 0; i < half; i++) { s += (w[i] & 0xF) - 8; s += ((w[i] >> 4) & 0xF) - 8; }
        w_sums[j] = s;
    }
}
...
static void matmul_q4(float *y, const uint8_t *q4_T, const float *scale,
    ...
    for (int j = 0; j < out_dim; j++) {
        ...
        for (int i = 0; i < half; i += 16) {
            ...
            __m128i w_low = _mm_unpacklo_epi8(lo, hi);
            __m128i w_high = _mm_unpackhi_epi8(lo, hi);
...
typedef struct {
    ...
    /* Q4 per-row: 4-bit packed (2 per byte) + per-row scale. NULL unless --q4. */
    uint8_t *q4_c_attn;   float *q4_c_attn_s;    int32_t *q4_c_attn_ws;
    uint8_t *q4_c_proj;   float *q4_c_proj_s;    int32_t *q4_c_proj_ws;
    uint8_t *q4_mlp_fc;   float *q4_mlp_fc_s;    int32_t *q4_mlp_fc_ws;
    uint8_t *q4_mlp_proj; float *q4_mlp_proj_s;  int32_t *q4_mlp_proj_ws;
} GPT2Layer;
...
typedef struct {
    ...
    float    *w_sign_float; /* OpenBLAS-style: pre-packed sign float for AVX2 FMA */
} SrvBinLayer;
...
static int load_binary_weights(const char *path) {
    ...
    for (int l = 0; l < N_LAYER; l++) {
        ...
        for (int mi = 0; mi < 4; mi++) {
            ...
            /* Also pre-pack as float for AVX2 FMA BWN path */
            {
                size_t nf = (size_t)bl->out_dim * bl->in_dim;
                bl->w_sign_float = malloc(nf * sizeof(float));
                for (int j = 0; j < bl->out_dim; j++) {
                    const uint64_t *wb = bl->wbits + (size_t)j * bl->n_words;
...
static void matmul_q8(float *y, const int8_t *q8_T, const float *scale,
    ...
    for (; j < out_dim; j++) {
        ...
        for (int i = 0; i < in_dim; i += 16) {
            int8x16_t xv = vld1q_s8((const int8_t*)(xq+i));
            xv = vsubq_s8(xv, vdupq_n_s8(128));
            acc = vdotq_s32(acc, xv, vld1q_s8(w+i));
        }
    ...
    for (; j < out_dim; j++) {
        ...
        for (int i = 0; i < in_dim; i += 8) {
            int8x8_t xv = vld1_s8((const int8_t*)(xq+i));
            xv = vsub_s8(xv, vdup_n_s8(128));
            acc = vpadalq_s16(acc, vmull_s8(xv, vld1_s8(w+i)));
        }
        y[j]=(float)(vaddvq_s32(acc))*x_scale*scale[j]+(b?b[j]:0);
    }
    return;
...
#elif defined(__ARM_NEON) && !defined(__aarch64__)
    /* ARMv7 NEON path: vmull_s8 8-output parallel.
     * ARMv7 has NEON but no SDOT and no vaddvq_s32.
     * Manual horizontal sum via vpadd_s32 + vget_lane_s32. */
    int j = 0;
    for (; j + 8 <= out_dim; j += 8) {
        int32x4_t a0=vdupq_n_s32(0),a1=vdupq_n_s32(0),a2=vdupq_n_s32(0),a3=vdupq_n_s32(0);
        int32x4_t a4=vdupq_n_s32(0),a5=vdupq_n_s32(0),a6=vdupq_n_s32(0),a7=vdupq_n_s32(0);
        ...
        #define HADDQ_S32(acc) ({ int32x2_t _p = vpadd_s32(vget_low_s32(acc), vget_high_s32(acc)); vget_lane_s32(_p,0)+vget_lane_s32(_p,1); })
        y[j+0]=(float)(HADDQ_S32(a0))*x_scale*scale[j+0]+(b?b[j+0]:0);
        y[j+1]=(float)(HADDQ_S32(a1))*x_scale*scale[j+1]+(b?b[j+1]:0);
        y[j+2]=(float)(HADDQ_S32(a2))*x_scale*scale[j+2]+(b?b[j+2]:0);
        y[j+3]=(float)(HADDQ_S32(a3))*x_scale*scale[j+3]+(b?b[j+3]:0);
        y[j+4]=(float)(HADDQ_S32(a4))*x_scale*scale[j+4]+(b?b[j+4]:0);
        y[j+5]=(float)(HADDQ_S32(a5))*x_scale*scale[j+5]+(b?b[j+5]:0);
        y[j+6]=(float)(HADDQ_S32(a6))*x_scale*scale[j+6]+(b?b[j+6]:0);
        y[j+7]=(float)(HADDQ_S32(a7))*x_scale*scale[j+7]+(b?b[j+7]:0);
        #undef HADDQ_S32
    }
    for (; j < out_dim; j++) {
        const int8_t *w = q8_T + (size_t)j * in_dim;
        ...
        int32_t s = vget_lane_s32(p,0) + vget_lane_s32(p,1);
        y[j]=(float)(s)*x_scale*scale[j]+(b?b[j]:0);
    }
    return;
}
#else
...
int main(int argc, char **argv) {
    ...
    if ((g_use_q8 || g_use_q4) && !g_use_lm_head_int8) {
        ...
        for (int l = 0; l < N_LAYER; l++) {
            ...
            if (g_use_q4) {
                L->q4_c_attn = malloc((size_t)N_EMBD*3*N_EMBD/2);
                L->q4_c_attn_s = malloc(3*N_EMBD*sizeof(float));
                L->q4_c_attn_ws = malloc(3*N_EMBD*sizeof(int32_t));
                quantize_q4(L->c_attn_w, L->q4_c_attn, L->q4_c_attn_s, N_EMBD, 3*N_EMBD);
                compute_q4_w_sums(L->q4_c_attn, L->q4_c_attn_ws, N_EMBD, 3*N_EMBD);
                L->q4_c_proj = malloc((size_t)N_EMBD*N_EMBD/2);
                L->q4_c_proj_s = malloc(N_EMBD*sizeof(float));
                L->q4_c_proj_ws = malloc(N_EMBD*sizeof(int32_t));
                quantize_q4(L->c_proj_w, L->q4_c_proj, L->q4_c_proj_s, N_EMBD, N_EMBD);
                compute_q4_w_sums(L->q4_c_proj, L->q4_c_proj_ws, N_EMBD, N_EMBD);
                L->q4_mlp_fc = malloc((size_t)N_EMBD*MLP_DIM/2);
                L->q4_mlp_fc_s = malloc(MLP_DIM*sizeof(float));
                L->q4_mlp_fc_ws = malloc(MLP_DIM*sizeof(int32_t));
                quantize_q4(L->mlp_fc_w, L->q4_mlp_fc, L->q4_mlp_fc_s, N_EMBD, MLP_DIM);
                compute_q4_w_sums(L->q4_mlp_fc, L->q4_mlp_fc_ws, N_EMBD, MLP_DIM);
                L->q4_mlp_proj = malloc((size_t)MLP_DIM*N_EMBD/2);
                L->q4_mlp_proj_s = malloc(N_EMBD*sizeof(float));
                L->q4_mlp_proj_ws = malloc(N_EMBD*sizeof(int32_t));
                quantize_q4(L->mlp_proj_w, L->q4_mlp_proj, L->q4_mlp_proj_s, MLP_DIM, N_EMBD);
                compute_q4_w_sums(L->q4_mlp_proj, L->q4_mlp_proj_ws, MLP_DIM, N_EMBD);
            }
