...
int main(int argc, char **argv) {
    ...
    printf("[*] steps:%d lr:%f ste:%d real_attn:%d logic:%d\n", n_steps, lr, use_ste, use_real_attn, use_logic);
    if (use_ste) {
        g_use_ste = 1;
    ...
    if (use_logic) {
        ...
        printf("[*] Logic-guided binarization: top 20%% norm → CORE (float), bottom 10%% → PRUNE (zero), middle 70%% → BINARY\n");
    }
    if (export_path) {
